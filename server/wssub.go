...
func (wss *WSS) Subscribe(w http.ResponseWriter, r *http.Request) (*Subscription, error) {
	...
	wss.log.Printf("No error in Subscribe...")
	ctx := r.Context()
	wss.log.Printf("context created")
	clientID := path.Base(r.URL.Path)
	room := path.Base(path.Dir(r.URL.Path))
	wss.log.Printf("room: %s", room)
	adapter := wss.rooms.Enter(room)
	wss.log.Printf("creating channel")
	ch := make(chan Message)
	wss.log.Printf("creating client")
	client := NewClientWithID(c, clientID)
	wss.log.Printf("new client, id: %s", client.id)
	...
	go func() {
		wss.log.Printf("go func, sssub::: %s, %s", clientID, room)
		defer func() {
